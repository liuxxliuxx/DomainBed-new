...
class SAGM_CUSTOM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, model, alpha, rho_scheduler, adaptive=False, perturb_eps=1e-12,
        ...
        defaults = dict(adaptive=adaptive, **kwargs)
        super(SAGM_CUSTOM, self).__init__(params, defaults)
        self.model = model
        self.base_optimizer = base_optimizer
        self.param_groups = self.base_optimizer.param_groups
        self.adaptive = adaptive
    ...
    @torch.no_grad()
    def basic_step(self, closure=None):
    ...
    @torch.no_grad()
    def update_step(self, grads=None,):

        # set new grads on model
