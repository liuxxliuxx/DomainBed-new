...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    for i, (env, _) in enumerate(in_splits + out_splits + test_splits):
        ...
        # evalmode="fast" 时，非 test env 的 in split 会被 evaluator 跳过，不必预建
        unused = args.evalmode == "fast" and i < n_in and i not in test_envs
        n_workers = 4 if args.prebuild_loader else dataset.N_WORKERS
        loader_kwargs = {"dataset": env, "batch_size": batchsize, "num_workers": n_workers}
        if args.prebuild_loader and not unused:
    ...
    # remove custom model weights before returning
    # Specify the path of the file to be deleted
    file_paths = [hparams["logdir"] / 'network_start.pt', hparams["logdir"] / "network_best.pt" ]
